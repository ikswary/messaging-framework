...
>
> 분산된 변수 수집 코드를 매번 짜는 대신, 도메인 관계를 그래프로 두고 부팅 때 수집 경로를 **미리 계산·검증.** 새 메시지마다 반복하던 데이터 조회 코드를 구조로 제거.
...
* **환경** — 여러 팀이 각자 운영하는 사내 프로덕트. 회원가입·결제·작업 알림 같은 반복적으로 발생하는 아웃바운드 메시지가 여러 마이크로서비스에 분산, 공통 규칙 없이 팀마다 그때그때 만든 코드가 적체.
* **반복 비용** — 새 메시지 추가마다 발생 조건 판단 → 흩어진 데이터 수집 → 템플릿 치환 → 채널 전송 → 에러·재전송을 처음부터 재구현.

  * 대부분 메시지마다 같은 로직이나 서비스별 중복 구현.
  * 메시지·채널 매핑 명세 부재 → 비슷한 메시지 추가 시 기존 코드 역추적 필요.
  * 특히 한 메시지가 여러 도메인(주문→고객·상품·배송 등)에 걸친 변수를 필요로 해, 그 분산 데이터 **수집 로직을 메시지마다 새로 작성** — 반복 비용의 가장 큰 몫.
* **목표** — 명세·구성·전송·추적을 한 프레임워크로 통합해 새 메시지 추가의 반복 작업을 구조적으로 효율화.

## ② 설계 / 구현

**설계의 두 축:**

* **레이어 분리** — 메시지 처리를 명세 → 구성 → 전송 → 추적 네 레이어로 분리, 레이어별 책임을 응집하고 인터페이스를 개방(기본 동작 + override).
* **분산 변수 수집** — 손쿼리 대신 공통 그래프 + 경로 계산을 통한 자동 쿼리.
...
* **역할** — 메시지 한 건의 정의를 `MessageSpec` 한 객체에 담음. 이후 모든 레이어가 이 객체를 읽어 동작.
* **단일 명세 근거**
  * **정의 소재지 일원화** — 한 메시지의 정의가 흩어지지 않고 한 객체에 모여, 찾고 고칠 곳이 하나로 고정
  * **데이터 주도 소비** — 새 메시지가 코드가 아니라 명세 값으로 추가돼, 레이어가 분기 없이 값만 읽음
...
* **특징**
  * **선언이 곧 명세** — 템플릿의 토큰 `{{CUSTOMER.email}}` 하나가 "이 변수를 채워라"는 지시. 명세를 적는 순간 수집할 데이터가 정해지고, **구성** 레이어가 이 토큰을 해석하여 액션.
...
* **그래프 채택 근거**
  ...
  * **런타임 비용 0** — 매 전송 때 드는 경로 탐색 비용이 들지 않음
  * **확장 저렴** — 새 도메인·특수 메시지가 늘어도 한 구현체로 흡수
* **특징**
  * **그래프 모델링·경로 계산** — 도메인 간 관계를 공통 그래프 한 벌로 모델링, 필요한 변수에서 거꾸로 따라가 수집 경로를 계산
  * **부팅 선계산** — 경로 계산을 매 전송이 아니라 부팅 때 1회만 하고 캐시
  * **확장점** — 특수 메시지는 composer 전략 주입, 새 도메인은 간선(`EdgeResolver`) 한 구현체 추가
...
**심화 — 부팅 경로 계산.** 변수에서 도메인을 거꾸로 따라가 경로를 확정:
...
* **얇은 위임 근거**
  * **오류 의미 분리** — 설정 오류와 일시적 전달 실패를 갈라, 재시도해도 소용없는 오류에 재시도 낭비를 막음
  * **책임 경계** — 재전송 판단은 추적이, 실제 전달은 채널이 맡아, 전송이 정책·상태를 떠안지 않음
  * **교체 용이** — 새 채널 도입·교체가 다른 레이어를 건드리지 않고 한 지점에서 끝남
* **특징**
  * **오류 신호 분리** — 등록 안 된 채널이면 즉시 예외, 네트워크 같은 일시적 실패만 `false`로 돌려 **추적**이 재전송을 판단
  * **전송은 직접, SDK만 분리** — 채널 선택·`send` 호출·성공/실패 판정은 프레임워크 코드, 외부 채널 SDK(Slack·Email·푸시)만 `Channel` 인터페이스 뒤로 격리
  * **무상태 디스패치** — `platform`으로 채널을 찾아 보내고 결과(`true`/`false`)만 **추적**에 넘김
...
* **결함 격리 근거**
  * **장애 전파 차단(가용성)** — 재전송이 모든 due 건을 함께 굴리는 공유 엔진 위에서 돌아, 한 건의 영구 실패가 그 엔진을 멈추면 그 뒤 재전송이 전부 정지 — 이 연쇄를 끊어 가용성 보존
* **특징**
  * **재전송 스케줄링(due-time scheduling)** — 다음 due 시각 계산 → 대기열 저장 → 주기 폴링(`RetransmissionScheduler`) → due 건 재주입
  * **결함 격리 메커니즘** — `supervisorScope`로 각 건을 독립 실행해 실패를 개별 scope에 격리, 실패 건은 버리지 않고 매 폴링마다 `onError`로 재노출
  * **조정점** — 재전송 동작(횟수·간격)은 코드 교체가 아니라 명세를 통해 데이터 주도 조정
...
프레임워크가 실질적으로 해소한 것 = **새 메시지 1건 추가 워크플로우.** 매번 반복하던 다섯 단계 수작업 → 선언 두 가지, 나머지는 프레임워크가 처리:
